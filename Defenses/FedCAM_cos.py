...
class Server:
    ...
    def compute_reconstruction_error(self, selected_clients):
        ...
        # Define Cosine Similarity module
        cos_sim = nn.CosineSimilarity(dim=1, eps=1e-6)

        # Compute cosine similarity
        cosine_similarity = cos_sim(act_flatten, gm_flatten)

        # Reshape cosine similarity to original shape (50, 10)
        cosine_similarity = cosine_similarity.view(50, -1)

        print("here shape gm", gm.shape,
              clients_act.shape, cosine_similarity.shape)

        res = -1*cosine_similarity.mean(dim=1)
        print("res", res.shape)
